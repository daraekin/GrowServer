...
import { PUNCH_ID_MAPPING, CLOTHING_MODS_MAPPING } from "./MassiveData";
...
export class Peer extends OldPeer<PeerData> {
  ...
  public getPunchID(): number {
    ...
    for (const slot of slots) {
      ...
      // Check massive mapping first
      if (PUNCH_ID_MAPPING[itemID]) {
        return PUNCH_ID_MAPPING[itemID];
      }

      // Fallback to wiki data if available in future
    }
  ...
  public formPlayMods() {
    ...
    Object.keys(this.data.clothing).forEach((k) => {
      ...
      let playMods = itemInfo?.playMods || [];

      // Combine with massive mapping data
      if (CLOTHING_MODS_MAPPING[itemID]) {
        playMods = [...playMods, ...CLOTHING_MODS_MAPPING[itemID]];
      }
      ...
      for (const mod of playMods) {
        ...
        if (name.includes("high jump")) charActive |= CharacterState.DOUBLE_JUMP; // Fallback for High Jump if not distinct?
